export async function encryptImage(
  ...
  const {
    ...
    extraCols,
    extraRows,
  } = padImageData(imageData, blockSize);
  ...
  const seed64 = hashArray
    .slice(0, 8)
    .reduce((acc, val, i) => acc + (val << (i * 8)), 0);
  ...
  const resultImage = permuteBlocks(paddedImageData, blockSize, permutation);
...
export async function decryptImage(
  imageData: { width: any; height: any; data?: any },
  blockSize: number,
  password: any,
  extraRows: number,
  extraCols: number
  ...
  for (let by = 0; by < heightInBlocks; by++) {
    for (let bx = 0; bx < widthInBlocks; bx++) {
      ...
      shiftBlock(
        ...
        x,
        y,
        (blockSize * blockSize - shift) % (blockSize * blockSize)
      );
    }
  }
...
function padImageData(
  ...
  if (!Number.isFinite(blockSize) || blockSize <= 0)
    throw new Error("blockSize inválido");
  if (!Number.isFinite(width) || !Number.isFinite(height))
    throw new Error("Dimensiones inválidas");
 ...
 */
async function permuteChannels(
  imageData: ImageData,
  blockSize: number,
  prng: { (): Promise<number>; (): Promise<number>; (): any },
  reverse: boolean
) {
  ...
  for (let by = 0; by < heightInBlocks; by++) {
    for (let bx = 0; bx < widthInBlocks; bx++) {
      ...
      const perm = reverse ? inversePermutations[val] : permutations[val];
 ...
 */
// async function applyNegativeTransform(
//   imageData: {
//     data: any;
//     width: number;
//     height: ImageDataSettings | undefined;
//   },
//   prng: () => any
// ) {
 ...
 */
async function applyBlockLevelNegativeTransform(
  imageData: ImageData,
  blockSize: number,
  prng: { (): Promise<number>; (): Promise<number>; (): any }
) {
